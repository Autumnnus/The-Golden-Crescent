"""Audit mainland Southeast Asian demographics and the sole Pegu transfer."""
...
def main() -> None:
    ...
    if len(target) != 28 or set(frozen) != target | {old_key} or new_key not in target:
    ...
    expected_states = deepcopy(source["states"])
    parts = expected_states[transfer["state"]]["split"]
    old_part = next(part for part in parts if part["owner"] == transfer["old_owner"])
    new_part = next(part for part in parts if part["owner"] == transfer["new_owner"])
    if old_part["provinces"] != [transfer["province"]] or transfer["province"] in new_part["provinces"]:
        errors.append("Pegu transfer baseline differs")
    old_part["provinces"].remove(transfer["province"])
    new_part["provinces"].append(transfer["province"])
    parts.remove(old_part)
    for state, old in source["states"].items():
        new = candidate["states"][state]
        expected = expected_states[state]
        if {k: v for k, v in expected.items() if k != "population"} != {k: v for k, v in new.items() if k != "population"}:
    ...
    for state_block in base.items:
        ...
        for owner_block in state_block.value.items:
            ...
            if share == old_key and other is None:
                continue
            if other is None:
    ...
    for tag, country in prior_report["countries"].items():
        if tag == transfer["old_owner"]:
            if report["countries"][tag]["population"] != country["population"] - audit["transferred_population"]:
                errors.append(f"{tag}: transferred enclave population differs")
        elif tag not in plan["countries"] and report["countries"][tag]["population"] != country["population"]:
            errors.append(f"{tag}: non-target population changed")
    if transfer["old_owner"] in report["states"][transfer["state"]]["owners"]:
        errors.append("Pegu retains Danish owner")
    if audit["transferred_population"] != sum(row["size"] for row in frozen[old_key]):
        errors.append("transferred population audit differs")
    ...
    result = {"passed": not errors, "countries": len(plan["countries"]), "shares": len(target),
              "cities": len(cities), "transferred_population": audit["transferred_population"],
