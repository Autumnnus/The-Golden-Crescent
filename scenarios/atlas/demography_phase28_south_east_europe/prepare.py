"""Build the Southern and Eastern European POP candidate."""
...
def main() -> None:
    ...
    parser.add_argument("--source", default="build/demography/south-east-europe-source.yml",
                        help="frozen pre-phase Atlas YAML")
    parser.add_argument("--out", default="build/demography/south-east-europe-candidate.yml")
    ...
    if len(shares) != 61 or set(shares) != set(frozen) or not release <= set(shares):
    ...
    world["title"] = "The Golden Crescent — 1836 siyasi dünya ve tamamlanmış demografi"
    world["description"] = (
        "1836 alternatif siyasi dünya ve önceki demografi korunur. İtalya, kuzey İberya, Balkanlar, "
        "Macaristan, Erdel, Tuna prenslikleri, Silezya, Doğu Prusya ve Kraków'da 28 ülkenin 61 "
        "doğrudan payı nüfus ve okuryazarlık girdisi alır; böylece dünyanın bütün state–ülke "
        "paylarında açık nüfus planı bulunur. Sınırlar değişmez."
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(HEADER + yaml.safe_dump(world, allow_unicode=True, sort_keys=False, width=120))
    (output.parent / "south-east-europe-audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2) + "\n")
