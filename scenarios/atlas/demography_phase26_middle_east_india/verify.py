"""Audit the Tranquebar correction, the Middle East–India POP plan and protected world data."""
...
def main() -> None:
    ...
    errors = []
    if len(target) != 31 or set(frozen) != target or set(plan["shares"]) != target:
    ...
    if report["states"]["STATE_MADRAS"]["owners"].get("DEN"):
        errors.append("DEN still owns Madras")
    ...
    for name, city in cities.items():
        ...
        parts = state_spec.get("split") or [{"owner": state_spec["owner"],
                                             "provinces": index["states"][city["state"]]["provinces"]}]
