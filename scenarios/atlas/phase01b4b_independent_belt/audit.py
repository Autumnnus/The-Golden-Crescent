"""Audit independent-belt production and formation materiel from vanilla scripts."""
...
def blank():
    return {
        "inputs": defaultdict(float), "outputs": defaultdict(float), "upkeep": defaultdict(float),
        "fleet_construction": defaultdict(float), "formations": defaultdict(int),
        "building_levels": 0, "building_records": 0, "army_max_manpower": 0, "navy_max_crew": 0,
    }
...
def main():
    ...
    rows = {tag: blank() for tag in COUNTRIES}
    sources = set()
    ...
    for state in scenario["states"].values():
        for tag in COUNTRIES:
            ...
            for spec in owner.get("buildings", {}).values():
                if not isinstance(spec, dict) or spec.get("level", 0) <= 0 or not spec.get("production_methods"):
                    continue
                level = spec["level"]
    ...
    result = {"scope": "six independent-belt economies: explicit PM output/input and full formation materiel upkeep", "countries": {}}
    for tag, row in rows.items():
        goods = {}
        for good in sorted(set(row["inputs"]) | set(row["outputs"]) | set(row["upkeep"])):
            output, inputs, upkeep = row["outputs"][good], row["inputs"][good], row["upkeep"][good]
            goods[good] = {
                "explicit_output": round(output, 2), "industrial_input": round(inputs, 2),
                "industry_net": round(output - inputs, 2), "formation_upkeep": round(upkeep, 2),
        ...
        result["countries"][tag] = {
            ...
            "army_max_manpower": row["army_max_manpower"], "navy_max_crew": row["navy_max_crew"],
            "goods": goods,
            "trade_dependencies": sorted(good for good, values in goods.items() if values["after_formation_upkeep"] < 0),
    ...
    for country in result["countries"].values():
        for good, values in country["goods"].items():
            regional[good] += values["after_formation_upkeep"]
    result["regional_after_formation_upkeep"] = {good: round(value, 2) for good, value in sorted(regional.items())}
    result["regional_external_dependencies"] = sorted(good for good, value in regional.items() if value < 0)

    result["source_sha256"] = {
        source: hashlib.sha256((mechanics.VANILLA / source).read_bytes()).hexdigest() for source in sorted(sources)
    }
    result["interpretation"] = {
        "capacity": "nameplate values from explicit selected production methods",
        "dependency": "negative local balance marks intended regional trade reliance, not an opening-day shortage forecast",
        "not_included": [
            "employment, qualifications, throughput and infrastructure", "prices, POP consumption and market access",
            "trade routes, tariffs, convoys and treaty enforcement", "wages, treasury, legitimacy and IG clout",
            "mobilization, readiness, replacement and fleet construction timing",
        ],
    }
