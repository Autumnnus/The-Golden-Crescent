"""Build the Nile, Horn and East Africa POP candidate."""
...
def main() -> None:
    ...
    parser.add_argument("--out", default="build/demography/nile-horn-east-africa-candidate.yml")
    ...
    if (len(states) != 57 or set(states) != set(frozen)
            or set(plan["joint_mix_overrides"]) - set(states)
            or set(plan["fixed_group_relabels"]) - set(states)):
    ...
    for tag, change in plan["country_culture_overrides"].items():
        if tag in world["countries"] or index["countries"][tag]["cultures"] != change["from"]:
            raise ValueError(f"{tag}: expected vanilla primary culture differs")
        world["countries"][tag] = {"cultures": change["to"]}
    ...
    for tag, laws in plan["country_law_additions"].items():
        if tag not in plan["countries"] or tag not in world["countries"] or "laws" in world["countries"][tag]:
            raise ValueError(f"{tag}: expected country without explicit laws differs")
    ...
    for key, design in states.items():
        state, tag = key.split("/")
        target = assigned[key]
        fixed, inherited, source_fixed = Counter(), Counter(), Counter()
        relabels = plan["fixed_group_relabels"].get(key, {})
        for row in frozen[key]:
            group = f"{row['culture']}/{row['religion']}"
            if row["pop_type"]:
                source_group = f"{group}/{row['pop_type']}"
                source_fixed[source_group] += row["size"]
                mapped = relabels.get(source_group, source_group)
                if mapped.split("/")[1:] != source_group.split("/")[1:]:
                    raise ValueError(f"{key}: relabel must change only culture, not religion/profession")
                fixed[mapped] += row["size"]
            else:
                inherited[group] += row["size"]
        if set(relabels) - set(source_fixed):
            raise ValueError(f"{key}: relabel source absent from frozen POPs")
        ...
        audit["shares"][key] = {"prior_population": by_country[tag][key], "population": target,
                                ...
                                "fixed_professions": dict(fixed), "source_fixed_professions": dict(source_fixed),
    ...
    world["title"] = "The Golden Crescent — 1836 siyasi dünya ve Nil-Boynuz-Doğu Afrika demografisi"
    world["description"] = (
        "1836 alternatif siyasi dünya ve önceki demografi korunur. Nil, Boynuz ve Doğu Afrika'daki "
        "54 ülkenin 16 state içindeki 57 doğrudan payına nüfus ve farklılaştırılmış "
        "okuryazarlık girdileri verilir. Habeş yaylasındaki yanlış Kikuyu köle POP "
        "etiketleri Oromo/Sidama olarak düzeltilir; Sennaar'a borç köleliği kanunu eklenir. "
        "Diğer ortak kültür-din grupları ve siyasi sınırlar korunur."
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(HEADER + yaml.safe_dump(world, allow_unicode=True, sort_keys=False, width=120))
    (output.parent / "nile-horn-east-africa-audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2) + "\n")
