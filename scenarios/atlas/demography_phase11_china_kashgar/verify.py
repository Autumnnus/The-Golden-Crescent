"""Audit the China/Kashgar candidate without changing protected POPs."""
...
def main() -> None:
    source = yaml.safe_load((ROOT / "build/demography/china-kashgar-source.yml").read_text())
    candidate = yaml.safe_load((ROOT / "build/demography/china-kashgar-candidate.yml").read_text())
    plan = yaml.safe_load((HERE / "plan.yml").read_text())
    frozen = json.loads((HERE / "source-pops.json").read_text())
    audit = json.loads((ROOT / "build/demography/china-kashgar-audit.json").read_text())
    prior_report = json.loads((ROOT / "build/demography/china-kashgar-source-report.json").read_text())
    report = json.loads((ROOT / "build/scenarios/china-kashgar-candidate/scenario-report.json").read_text())
    index = json.loads((ROOT / "build/index.json").read_text())
    target = set(audit["shares"])
    errors = []
    if len(target) != 34 or set(frozen) != target:
    ...
    for tag, prior_country in source["countries"].items():
        current = candidate["countries"].get(tag)
        if current is None:
            errors.append(f"{tag}: country removed")
            continue
        expected = dict(prior_country)
        if tag in plan["country_primary_culture_additions"]:
            expected["cultures"] = prior_country["cultures"] + plan["country_primary_culture_additions"][tag]
        if expected != current:
            errors.append(f"{tag}: country definition changed beyond planned culture")
    ...
    base = pdx.parse_file(ROOT / "build/demography/china-kashgar-source-pops.txt").get_node("POPS")
    actual = pdx.parse_file(ROOT / "build/scenarios/china-kashgar-candidate/common/history/pops/tgc_pops.txt").get_node("POPS")
    ...
    core = sum(audit["shares"][f"{state}/JNG"]["population"] for state in plan["jiangnan_lower_yangtze_core"])
    if core != audit["jiangnan_core_population"] or not plan["jiangnan_core_range"][0] <= core <= plan["jiangnan_core_range"][1]:
        errors.append("Jiangnan lower Yangtze core outside revised target")
    ...
    result = {"passed": not errors, "countries": len(plan["countries"]), "shares": len(target),
              "cities": len(cities), "jiangnan_core_population": core,
              "world_population": sum(row["population"] for row in report["states"].values()), "errors": errors}
    out = ROOT / "build/demography/china-kashgar-verification.json"
