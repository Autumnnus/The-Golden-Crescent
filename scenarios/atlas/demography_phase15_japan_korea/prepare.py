"""Build the Japan, Korea, Ryukyu and Ezo-Sakhalin POP candidate."""
...
def main() -> None:
    ...
    parser.add_argument("--out", default="build/demography/japan-korea-candidate.yml")
    ...
    if len(states) != 22 or set(states) != set(frozen) or set(plan["joint_mix_overrides"]) - set(states):
    ...
    world["title"] = "The Golden Crescent — 1836 siyasi dünya ve Japonya-Kore demografisi"
    world["description"] = (
        "1836 alternatif siyasi dünya ve önceki demografi korunur. Japonya, Kore, Ryukyu ve Ezo-Sahalin'deki "
        "yedi ülkenin 22 doğrudan state payına nüfus, ortak kültür-din ve okuryazarlık girdileri verilir."
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(HEADER + yaml.safe_dump(world, allow_unicode=True, sort_keys=False, width=120))
    (output.parent / "japan-korea-audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2) + "\n")
