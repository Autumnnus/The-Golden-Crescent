"""Build the North American POP candidate on top of the Alaska/Texas political correction."""
...
def main() -> None:
    ...
    parser.add_argument("--source", default="build/demography/north-america-political.yml",
                        help="political-correction Atlas YAML built before this demography phase")
    parser.add_argument("--out", default="build/demography/north-america-candidate.yml")
    ...
    if len(shares) != 95 or set(shares) != set(frozen) or not release <= set(shares):
    ...
    for state, cultures in state_groups.items():
        spec = world["states"][state]
        if {part["owner"] for part in spec["split"]} != {key.split("/")[1] for key in shares
                                                          if key.startswith(state + "/")}:
            raise ValueError(f"{state}: homeland rule needs every owner share in this plan")
    ...
    world["title"] = "The Golden Crescent — 1836 siyasi dünya ve Kuzey Amerika demografisi"
    world["description"] = (
        "1836 alternatif siyasi dünya ve önceki demografi korunur. Alaska kıyısındaki Rus "
        "şirketi ile Teksas cumhuriyetinin payları yerel Tlingit–Unangan ve Caddo "
        "yönetimlerine geçer. Endülüs Meksikası dışındaki Kuzey Amerika'da 45 ülkenin 95 "
        "doğrudan payı nüfus ve okuryazarlık girdisi alır: kıyı kolonileri yerleşimci "
        "çoğunluğunu korur, iç kıtanın yerel devletleri devralınmış ABD/Kanada yerleşimci "
        "nüfusu yerine kendi halklarıyla başlar."
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(HEADER + yaml.safe_dump(world, allow_unicode=True, sort_keys=False, width=120))
    (output.parent / "north-america-audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2) + "\n")
