"""Freeze Middle Eastern and remaining Indian POPs after the Tranquebar correction."""
...
POPS = ROOT / "build/scenarios/middle-east-india-political/common/history/pops/tgc_pops.txt"
...
def main() -> None:
    ...
    if len(targets) != 31 or {key.split("/")[1] for key in targets} != set(plan["countries"]):
    ...
    # Absorbed pre-correction shares come from the pre-phase active POPs.
    before = pdx.parse_file(ROOT / "build/demography/middle-east-india-source-pops.txt").get_node("POPS")
    found["absorbed"] = {}
    for target, absorbed in plan["absorb"].items():
        state, tag = absorbed.split("/")
        block = before.get_node("s:" + state).get_node("region_state:" + tag)
        found["absorbed"][absorbed] = [{"culture": pop.get_str("culture"), "religion": pop.get_str("religion"),
                                        "pop_type": pop.get_str("pop_type"), "size": pop.get_int("size")}
                                       for pop in block.getall("create_pop")]
