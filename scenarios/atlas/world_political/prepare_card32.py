...
def main():
 p=R/'build/political-uca-catalog.json';subprocess.run([sys.executable,'scripts/tools.py','atlas','catalog','--region','UCA','--out',str(p)],cwd=R,check=True);by={s['id']:s for s in json.loads(p.read_text())['states']};st={}
