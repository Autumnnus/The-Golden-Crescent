...
2. All content was **empirically verified against Victoria 3 v1.13.9 (Matcha)** game files Windows: (`C:\Program Files (x86)\Steam\steamapps\common\Victoria 3\game`) Mac: (`~/Library/Application Support/Steam/steamapps/common/Victoria 3/game`) and the "Victorian Century" mod in August 2026. Anything the authors could not verify is explicitly marked `(unverified)` in the docs.
...
3. Named modifiers used by `add_modifier` live in **`common/static_modifiers/`** (the old `common/modifiers/` folder no longer exists). Modifier _types_ are in `common/modifier_type_definitions/`.
...
8. Debug with logs (Windows: `...\Documents\Paradox Interactive\Victoria 3\logs\`, Mac: `~/Documents/Paradox Interactive/Victoria 3/logs/`): `error.log` (script errors with file:line), `game.log`, `database_conflicts.log` ("Overriding entry" lines show what your mod replaced).
