"""Build the Russia, Baltic, Finland and Siberia POP candidate on the Sakhalin correction."""
...
def main() -> None:
    ...
    parser.add_argument("--source", default="build/demography/russia-siberia-political.yml",
                        help="political-correction Atlas YAML built before this demography phase")
    parser.add_argument("--out", default="build/demography/russia-siberia-candidate.yml")
    ...
    if len(shares) != 56 or set(shares) != set(frozen) or not release <= set(shares):
    ...
    world["title"] = "The Golden Crescent — 1836 siyasi dünya ve Rusya–Sibirya demografisi"
    world["description"] = (
        "1836 alternatif siyasi dünya ve önceki demografi korunur. Sahalin'deki Rus şirketi payı "
        "Ainu Mosir'e katılır. Moskova, Novgorod, Baltık, İsveç Finlandiyası, Büyük Tatar Hanlığı, "
        "Kırım, Volga–Ural meclisleri ve Sibirya'da 20 ülkenin 56 doğrudan payı nüfus ve "
        "okuryazarlık girdisi alır; Rus imparatorluk yayılımının yerleşimci nüfusu yerel halklara "
        "döner."
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(HEADER + yaml.safe_dump(world, allow_unicode=True, sort_keys=False, width=120))
    (output.parent / "russia-siberia-audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2) + "\n")
