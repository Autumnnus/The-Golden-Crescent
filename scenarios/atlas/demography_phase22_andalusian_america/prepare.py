"""Build the Andalusian America (Mexico, Central America, Caribbean) POP candidate."""
...
def main() -> None:
    ...
    parser.add_argument("--source", default="build/demography/andalusian-america-source.yml",
                        help="frozen pre-phase Atlas YAML")
    parser.add_argument("--out", default="build/demography/andalusian-america-candidate.yml")
    ...
    if len(shares) != 35 or set(shares) != set(frozen) or not release <= set(shares):
    ...
    world["title"] = "The Golden Crescent — 1836 siyasi dünya ve Endülüs Amerika'sı demografisi"
    world["description"] = (
        "1836 alternatif siyasi dünya ve önceki demografi korunur. Meksika, Orta Amerika ve "
        "Karayipler'de 26 ülkenin 35 doğrudan payı nüfus ve okuryazarlık girdisi alır: Yeni "
        "Endülüs Nahua çoğunluğu ve Sünni/Katolik kreol toplulukla, yerel meclisler kendi "
        "halklarıyla, İnci Adaları korunan kölelik düzeniyle başlar. Sınırlar ve diplomasi "
        "değişmez."
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(HEADER + yaml.safe_dump(world, allow_unicode=True, sort_keys=False, width=120))
    (output.parent / "andalusian-america-audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2) + "\n")
