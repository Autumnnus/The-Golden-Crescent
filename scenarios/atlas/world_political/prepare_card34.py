...
def main():
 p=R/'build/political-dei-west-java-catalog.json';subprocess.run([sys.executable,'scripts/tools.py','atlas','catalog','--region','DEI','--out',str(p)],cwd=R,check=True);s=next(x for x in json.loads(p.read_text())['states'] if x['id']=='STATE_WEST_JAVA');parts=[{'owner':'YOG' if e['tag']=='DEI' else e['tag'],'provinces':['x'+q[1:].upper() for q in dict.fromkeys(e['provinces'])]} for e in s['owners']]
