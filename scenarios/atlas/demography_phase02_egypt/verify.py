"""Audit Mısır POP output, protected legacy professions, cities and unchanged map."""
...
CANDIDATE = ROOT / "build/demography/egypt-candidate.yml"
EXPECTED = ROOT / "build/demography/egypt-plan-audit.json"
BASE_POPS = ROOT / "common/history/pops/tgc_pops.txt"
NEW_POPS = ROOT / "build/scenarios/egypt-demography-candidate/common/history/pops/tgc_pops.txt"
REPORT = ROOT / "build/demography/egypt-candidate-report.json"
OUT = ROOT / "build/demography/egypt-verification.json"
...
def main() -> None:
    ...
    for state, old in source["states"].items():
        ...
        old_other = {tag: value for tag, value in old.get("population", {}).get("by_owner", {}).items() if tag != "EGY"}
        new_other = {tag: value for tag, value in new.get("population", {}).get("by_owner", {}).items() if tag != "EGY"}
    ...
    for block in baseline.items:
        ...
        for item in block.value.items:
            if block.key.removeprefix("s:") in changed_states and item.key == "region_state:EGY":
                continue
    ...
    for state, plan in expected["states"].items():
        node = emitted.get_node("s:" + state).get_node("region_state:EGY")
        ...
        for key, size in plan["fixed_legacy_groups"].items():
            ...
            if len(key.split("/")) == 3:
                fixed_total += size
        if report["states"][state]["owners"]["EGY"]["population"] != plan["population"]:
            errors.append(f"{state}: report population differs")
        if report["states"][state]["owners"]["EGY"]["settings"].get("literacy") != plan["literacy"]:
    ...
    for city, profile in cities.items():
        ...
        if state not in actual_joint:
            errors.append(f"{city}: city outside Egypt phase")
            continue
    ...
    if report["countries"]["EGY"]["population"] != expected["target_population"]:
        errors.append("Egypt country population differs")
    literacy = sum(row["population"] * row["literacy"] for row in expected["states"].values()) / expected["target_population"]
    if not .34 <= literacy <= .40:
        errors.append("weighted literacy outside written Egypt target")
    if slave_total != 189833:
        errors.append("inherited enslaved POP headcount changed before law review")
    result = {
        ...
        "legacy_enslaved_population": slave_total, "reviewed_city_hubs": len(cities),
