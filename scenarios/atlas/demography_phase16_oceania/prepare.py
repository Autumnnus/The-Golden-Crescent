"""Build the indigenous Oceania POP candidate after the Samoa correction."""
...
def main() -> None:
    ...
    parser.add_argument("--source", default="build/demography/oceania-political-source.yml",
                        help="pre-phase Atlas YAML; use the frozen build/demography source after activation")
    parser.add_argument("--out", default="build/demography/oceania-candidate.yml")
    ...
    world["title"] = "The Golden Crescent — 1836 siyasi dünya ve Okyanusya demografisi"
    world["description"] = (
        "1836 alternatif siyasi dünya ve önceki demografi korunur. Samoa yerel payı Tonga'dan ayrılır; "
        "Okyanusya'daki 37 ülkenin 56 doğrudan state payına nüfus, ortak kültür-din ve okuryazarlık girdileri verilir."
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(HEADER + yaml.safe_dump(world, allow_unicode=True, sort_keys=False, width=120))
    (output.parent / "oceania-audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2) + "\n")
