...
def main():
 p=R/'build/political-aus-catalog.json';subprocess.run([sys.executable,'scenarios/atlas/world_political/catalog_vanilla.py','atlas','catalog','--baseline','vanilla','--region','AUS','--out',str(p)],cwd=R,check=True);by={s['id']:s for s in json.loads(p.read_text())['states']};st={}
