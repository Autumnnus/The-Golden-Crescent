"""Audit three British crowns, other POP blocks, fixed professions and cities."""
...
CANDIDATE = ROOT / "build/demography/british-crowns-candidate.yml"
EXPECTED = ROOT / "build/demography/british-crowns-plan-audit.json"
BASE_POPS = ROOT / "common/history/pops/tgc_pops.txt"
NEW_POPS = ROOT / "build/scenarios/british-crowns-demography-candidate/common/history/pops/tgc_pops.txt"
REPORT = ROOT / "build/demography/british-crowns-candidate-report.json"
OUT = ROOT / "build/demography/british-crowns-verification.json"
...
def main() -> None:
    ...
    result = {
        ...
        "crown_count": len(expected["countries"]), "state_shares": len(expected["states"]),
        "population": sum(d["population"] for d in expected["countries"].values()),
        "crown_literacy_inputs": member_literacy,
