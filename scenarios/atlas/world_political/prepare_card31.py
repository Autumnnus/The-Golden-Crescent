...
def main():
 p=R/'build/political-clm-catalog.json';subprocess.run([sys.executable,'scripts/tools.py','atlas','catalog','--region','CLM','--out',str(p)],cwd=R,check=True);s=next(x for x in json.loads(p.read_text())['states'] if x['id']=='STATE_CUNDINAMARCA');parts=[{'owner':'VMU' if e['tag']=='CLM' else e['tag'],'provinces':['x'+q[1:].upper() for q in dict.fromkeys(e['provinces'])]} for e in s['owners']]
