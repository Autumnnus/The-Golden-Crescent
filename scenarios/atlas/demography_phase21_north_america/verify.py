"""Audit the North American political correction, POP plan and protected world data."""
...
from political import COUNTRIES, TRANSFERS  # noqa: E402

BASE = "north-america"
POLITICAL_STATES = {state for state, *_ in TRANSFERS}
RENAMED = {f"{state}/{old}": f"{state}/{new}" for state, old, new, _ in TRANSFERS}
...
def main() -> None:
    ...
    errors = []
    if len(target) != 95 or set(frozen) != target or set(plan["shares"]) != target:
    ...
    for state, old_tag, new_tag, count in TRANSFERS:
        ...
        new_parts = [(new_tag if o == old_tag else o, p) for o, p in old_parts]
        if [(p["owner"], p["provinces"]) for p in political["states"][state]["split"]] != new_parts:
            errors.append(f"{state}: split differs beyond {old_tag}->{new_tag}")
        if sum(len(p) for o, p in old_parts if o == old_tag) != count:
            errors.append(f"{state}: transferred province count differs")
    if set(political["countries"]) != set(source["countries"]) | set(COUNTRIES):
        errors.append("political country set differs")
    for tag, spec in political["countries"].items():
        if spec != (COUNTRIES.get(tag) or source["countries"][tag]):
    ...
    if slave_tags != set(plan["country_laws"]):
        errors.append(f"slave POP holders differ from planned slavery-law countries: {sorted(slave_tags)}")
    ...
    for tag, row in audit["countries"].items():
        ...
        if tag in {new for _, _, new, _ in TRANSFERS}:
            outside = 0
        if outside:
    ...
    if "TEX" in report["countries"] or report["countries"]["ALK"]["population"] != 793:
        errors.append("TEX/ALK residual country state differs")
