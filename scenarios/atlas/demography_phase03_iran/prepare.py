"""Prepare the seven Iran confederation members' joint POP plans."""
...
def main() -> None:
    ...
    parser.add_argument("--out", default="build/demography/iran-candidate.yml")
    ...
    if set(plan["states"]) != set(legacy):
        raise ValueError("Iran plan and frozen legacy state shares differ")
    ...
    for tag, row in plan["countries"].items():
        if sum(d["population"] for d in plan["states"].values() if d["owner"] == tag) != row["population"]:
            raise ValueError(f"{tag}: state shares do not add to member target")

    world["title"] = "The Golden Crescent — 1836 siyasi dünya ve üç çekirdek toplum"
    world["description"] = (
        "1836 alternatif siyasi dünya: sınır ve bağlılıklar. Rûm, Mısır ve İran "
        "konfederasyonunun yedi üyesinde nüfus, ortak kültür-din ve okuryazarlık "
        "düzenlendi; diğer bölgelerde geçici vanilla nüfus aktarımı sürer. "
        "Mevcut köle ve meslekli POP türleri korunur."
    )
    ...
    for state, design in plan["states"].items():
        ...
        if set(legacy[state]) != {owner}:
            raise ValueError(f"{state}: frozen owner differs")
        mix = design["mix"]
    ...
    (output.parent / "iran-plan-audit.json").write_text(
        json.dumps(audit, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    print(f"wrote {output.relative_to(ROOT)} ({len(audit['states'])} shares, {len(plan['countries'])} members)")
