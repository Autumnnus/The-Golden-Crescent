"""Freeze all targeted mainland Southeast Asian POPs and the Danish Pegu enclave."""
...
def main() -> None:
    ...
    targets.add(f"{transfer['state']}/{transfer['old_owner']}")
    if len(targets) != 29 or {key.split("/")[1] for key in targets} != set(plan["countries"]) | {"DEN"}:
