"""Build the phase-12 candidate from frozen POPs and explicit joint mixes."""
...
def main() -> None:
    ...
    parser.add_argument("--out", default="build/demography/himalaya-steppe-candidate.yml")
    ...
    if len(states) != 13 or set(states) != set(frozen) or set(plan["composition_overrides"]) - set(states):
    ...
    for key, design in states.items():
        ...
        override = plan["composition_overrides"].get(key)
    ...
    world["title"] = "The Golden Crescent — 1836 siyasi dünya ve Himalaya–bozkır demografisi"
    world["description"] = (
        "1836 alternatif siyasi dünya ve önceki demografi korunur. Tibet, Cungarya, Moğol hanlıkları, "
        "Nepal, Bhutan ve Sikkim'in 13 doğrudan state payında nüfus, ortak kültür-din ve okuryazarlık girdileri."
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(HEADER + yaml.safe_dump(world, allow_unicode=True, sort_keys=False, width=120))
    (output.parent / "himalaya-steppe-audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2) + "\n")
