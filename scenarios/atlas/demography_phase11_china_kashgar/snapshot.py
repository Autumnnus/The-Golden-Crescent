"""Freeze the current generated POP blocks for China and Kashgar."""
...
def main() -> None:
    ...
    targets = {f"{state}/{spec['owner']}" for state, spec in plan["states"].items()}
    if len(targets) != 34 or {key.split("/")[1] for key in targets} != set(plan["countries"]):
