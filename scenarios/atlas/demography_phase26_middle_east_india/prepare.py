"""Build the Middle East and remaining India POP candidate on the Tranquebar correction."""
...
def owners_of(spec: dict) -> set[str]:
...
def main() -> None:
    ...
    parser.add_argument("--source", default="build/demography/middle-east-india-political.yml",
                        help="political-correction Atlas YAML built before this demography phase")
    parser.add_argument("--out", default="build/demography/middle-east-india-candidate.yml")
    ...
    for key, design in shares.items():
        ...
        if tag not in owners_of(spec) or spec.get("pops") != "inherit":
        ...
        if key in plan["absorb"]:
            # Keep the existing plan's groups exactly and add the absorbed share's frozen POPs.
            old = spec["population"]["by_owner"][tag]
            counts = Counter(apportion(old["total"], {
                "/".join([row["culture"], row["religion"]] + ([row["pop_type"]] if row.get("pop_type") else [])):
                round(row["share"] * 10**9) for row in old["composition"]}))
            for row in absorbed_pops[plan["absorb"][key]]:
                counts["/".join([row["culture"], row["religion"]] + ([row["pop_type"]] if row["pop_type"] else []))] += row["size"]
            fixed = Counter({g: c for g, c in counts.items() if g.count("/") == 2})
            if sum(counts.values()) != design["population"] or old["literacy"] != design["literacy"]:
                raise ValueError(f"{key}: absorbed total or literacy differs from the plan")
            counts = dict(counts)
        else:
    ...
    world["title"] = "The Golden Crescent — 1836 siyasi dünya ve Ortadoğu–Hindistan kalıntı demografisi"
    world["description"] = (
        "1836 alternatif siyasi dünya ve önceki demografi korunur. Madras'taki Danimarka "
        "Tranquebar payı Tamil krallığına katılır. Rûm bağlıları, Kürdistan, Levant, Arabistan, "
        "Yemen ve Körfez ile son Hint ve Hint Okyanusu paylarında 25 ülkenin 31 doğrudan payı "
        "nüfus ve okuryazarlık girdisi alır."
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(HEADER + yaml.safe_dump(world, allow_unicode=True, sort_keys=False, width=120))
    (output.parent / "middle-east-india-audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2) + "\n")
