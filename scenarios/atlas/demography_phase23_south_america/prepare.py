"""Build the South American POP candidate."""
...
def main() -> None:
    ...
    parser.add_argument("--source", default="build/demography/south-america-source.yml",
                        help="frozen pre-phase Atlas YAML")
    parser.add_argument("--out", default="build/demography/south-america-candidate.yml")
    ...
    if len(shares) != 63 or set(shares) != set(frozen) or not release <= set(shares):
    ...
    world["title"] = "The Golden Crescent — 1836 siyasi dünya ve Amerika demografisi"
    world["description"] = (
        "1836 alternatif siyasi dünya ve önceki demografi korunur. Güney Amerika'da 38 "
        "ülkenin 63 doğrudan payı nüfus ve okuryazarlık girdisi alır: And krallıkları, Muisca, "
        "Guaraní, Mapuçe, Pampa ve Amazon yönetimleri kendi halklarıyla, Fas Brezilyası ve Yeni "
        "İşbiliye yasal kölelik düzeniyle başlar. Sınırlar ve diplomasi değişmez."
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(HEADER + yaml.safe_dump(world, allow_unicode=True, sort_keys=False, width=120))
    (output.parent / "south-america-audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2) + "\n")
