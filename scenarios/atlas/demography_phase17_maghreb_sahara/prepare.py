"""Build the Maghreb and Sahara POP candidate."""
...
def main() -> None:
    ...
    parser.add_argument("--out", default="build/demography/maghreb-sahara-candidate.yml")
    ...
    if len(states) != 40 or set(states) != set(frozen) or set(plan["joint_mix_overrides"]) - set(states):
    ...
    world["title"] = "The Golden Crescent — 1836 siyasi dünya ve Mağrip-Sahra demografisi"
    world["description"] = (
        "1836 alternatif siyasi dünya ve önceki demografi korunur. Mağrip ve Sahra'daki "
        "24 ülkenin 40 doğrudan state payına nüfus, ortak kültür-din ve okuryazarlık girdileri; "
        "Tuareg ülkesine kurulu oyunun tuareg kültürü verilir."
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(HEADER + yaml.safe_dump(world, allow_unicode=True, sort_keys=False, width=120))
    (output.parent / "maghreb-sahara-audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2) + "\n")
