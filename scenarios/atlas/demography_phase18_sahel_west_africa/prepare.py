"""Build the Sahel and West Africa POP candidate."""
...
def main() -> None:
    ...
    parser.add_argument("--out", default="build/demography/sahel-west-africa-candidate.yml")
    ...
    if len(states) != 56 or set(states) != set(frozen) or set(plan["joint_mix_overrides"]) - set(states):
    ...
    world["title"] = "The Golden Crescent — 1836 siyasi dünya ve Sahel-Batı Afrika demografisi"
    world["description"] = (
        "1836 alternatif siyasi dünya ve önceki demografi korunur. Sahel ve Batı Afrika'daki "
        "41 ülkenin 23 state içindeki 56 doğrudan payına nüfus, mevcut ortak kültür-din "
        "gruplarını koruyan kompozisyon ve farklılaştırılmış okuryazarlık girdileri verilir. "
        "Siyasi sınırlar ve ülke kimlikleri değişmez."
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(HEADER + yaml.safe_dump(world, allow_unicode=True, sort_keys=False, width=120))
    (output.parent / "sahel-west-africa-audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2) + "\n")
