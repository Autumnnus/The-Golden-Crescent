"""Build the Western European POP candidate."""
...
def main() -> None:
    ...
    parser.add_argument("--source", default="build/demography/west-europe-source.yml",
                        help="frozen pre-phase Atlas YAML")
    parser.add_argument("--out", default="build/demography/west-europe-candidate.yml")
    ...
    if len(shares) != 84 or set(shares) != set(frozen) or not release <= set(shares):
    ...
    world["title"] = "The Golden Crescent — 1836 siyasi dünya ve Batı Avrupa demografisi"
    world["description"] = (
        "1836 alternatif siyasi dünya ve önceki demografi korunur. Altı Fransız devleti, Alman "
        "prenslikleri, Bohemya, Avusturya, Alçak Ülkeler, İsviçre ve Kalmar taçlarında 47 ülkenin "
        "84 doğrudan payı nüfus ve farklılaştırılmış okuryazarlık girdisi alır; dar kimlik "
        "düzeltmeleri ve küçük İslam dünyası tüccar toplulukları eklenir. Sınırlar değişmez."
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(HEADER + yaml.safe_dump(world, allow_unicode=True, sort_keys=False, width=120))
    (output.parent / "west-europe-audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2) + "\n")
