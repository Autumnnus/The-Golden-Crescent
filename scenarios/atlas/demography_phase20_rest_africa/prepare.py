"""Build the Central and Southern Africa POP candidate."""
...
def main() -> None:
    ...
    parser.add_argument("--out", default="build/demography/central-southern-africa-candidate.yml")
    ...
    if len(states) != 72 or set(states) != set(frozen) or set(plan["joint_mix_overrides"]) - set(states):
    ...
    world["title"] = "The Golden Crescent — 1836 siyasi dünya ve Orta-Güney Afrika demografisi"
    world["description"] = (
        "1836 alternatif siyasi dünya ve önceki demografi korunur. Orta ve Güney Afrika'daki "
        "47 ülkenin 30 state içindeki 72 doğrudan payına nüfus ve farklılaştırılmış "
        "okuryazarlık girdileri verilir. Dört güney payında koloni devriyle kalmış "
        "aşırı Britanyalı/Boer oranları küçük topluluklara indirilir; diğer ortak "
        "kültür-din grupları ve siyasi sınırlar korunur."
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(HEADER + yaml.safe_dump(world, allow_unicode=True, sort_keys=False, width=120))
    (output.parent / "central-southern-africa-audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2) + "\n")
